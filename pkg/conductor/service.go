...
import (
	...
	opchealth "github.com/ethereum-optimism/optimism/op-conductor/health"
...
func New(ctx context.Context, cfg *opconductor.Config, log log.Logger, version string) (*OpConductor, error) {
...
func (c *OpConductor) initHealthMonitor(ctx context.Context) error {
	...
	// Use our custom health monitor that doesn't check peer stats
	c.hmon = health.NewBootstrapHealthMonitor(
		c.log,
		c.metrics,
...
func (oc *OpConductor) action() {
	...
	case status.leader && !status.healthy && !status.active:
		...
		if !oc.prevState.leader && !oc.prevState.active && !errors.Is(oc.hcerr, opchealth.ErrSequencerConnectionDown) {
	...
	case status.leader && !status.healthy && status.active:
		...
		if oc.prevState.leader && !oc.prevState.healthy && !oc.prevState.active && !errors.Is(oc.hcerr, opchealth.ErrSequencerConnectionDown) {
...
func (oc *OpConductor) startSequencer() error {
	...
	if errors.Is(err, ErrNoUnsafeHead) {
		...
		// Get the latest unsafe block from execution layer
		unsafeInNode, err = oc.ctrl.LatestUnsafeBlock(ctx)
		if err != nil {
			return errors.Wrap(err, "failed to get latest unsafe block from execution layer")
		}

		oc.log.Info("Auto-bootstrapping sequencer from execution layer",
			"hash", unsafeInNode.Hash(),
			"number", unsafeInNode.NumberU64())

		// Start sequencer with the execution layer's unsafe head
		err = oc.ctrl.StartSequencer(ctx, unsafeInNode.Hash())
		if err != nil {
